...
SELECT A.DEPT_NAME, COALESCE(NUM_STUDENT,0) AS student_number 
FROM DEPARTMENT A LEFT JOIN (SELECT DISTINCT DEPT_ID, COUNT(STUDENT_ID) OVER(PARTITION BY DEPT_ID) AS NUM_STUDENT
FROM STUDENT) B
