...
SELECT exam_id, student_id, 
    DENSE_RANK() OVER(PARTITION BY EXAM_ID ORDER BY SCORE) AS LOW_RANK,
    DENSE_RANK() OVER(PARTITION BY EXAM_ID ORDER BY SCORE DESC) AS HIGH_RANK
FROM EXAM)


SELECT DISTINCT B.* FROM EXAM A
INNER JOIN STUDENT B
ON A.STUDENT_ID=B.STUDENT_ID
WHERE A.STUDENT_ID NOT IN (SELECT DISTINCT student_id FROM CTE
