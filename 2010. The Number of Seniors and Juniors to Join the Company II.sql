...
WITH GET_SENIOR AS(
SELECT * FROM (
    SELECT EMPLOYEE_ID, 70000-SUM(SALARY) OVER(ORDER BY SALARY) AS BUDGET_LEFT
    FROM CANDIDATES
...
GET_JUNIOR AS(
    SELECT * FROM (
        SELECT employee_id, 
            (SELECT COALESCE(MIN(BUDGET_LEFT),70000) FROM GET_SENIOR)-SUM(SALARY) OVER(ORDER BY SALARY) AS BUDGET_LEFT FROM CANDIDATES
