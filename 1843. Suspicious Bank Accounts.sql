...
(SELECT 
    ACCOUNT_ID,
    DATE_FORMAT(DAY,'%Y%m') AS MONTH_DATE,
    COALESCE(SUM(CASE WHEN TYPE='Creditor' THEN AMOUNT END),0) AS INCOME
FROM TRANSACTIONS
GROUP BY 1,2
ORDER BY 1,2)

SELECT DISTINCT ACCOUNT_ID FROM (SELECT 
    A.ACCOUNT_ID,
    INCOME,
    LEAD(INCOME,1) OVER(PARTITION BY ACCOUNT_ID ORDER BY MONTH_DATE) AS NEXT_INCOME,
    MONTH_DATE,
    LEAD(MONTH_DATE,1) OVER(PARTITION BY ACCOUNT_ID ORDER BY MONTH_DATE) AS NEXT_MONTH,       
    B.MAX_INCOME
FROM CTE A
INNER JOIN ACCOUNTS B
ON A.ACCOUNT_ID=B.ACCOUNT_ID) AS A
WHERE A.INCOME>MAX_INCOME AND NEXT_INCOME>MAX_INCOME 
AND NEXT_MONTH-MONTH_DATE=1