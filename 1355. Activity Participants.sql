...
WITH CTE AS(
SELECT A.NAME, 
    DENSE_RANK() OVER(ORDER BY COUNT(B.NAME) DESC) AS TOP_ACTIVITY,
    DENSE_RANK() OVER(ORDER BY COUNT(B.NAME)) AS BOTTOM_ACTIVITY
FROM Activities A
LEFT JOIN FRIENDS B
...
FROM CTE 
WHERE TOP_ACTIVITY<>1 AND BOTTOM_ACTIVITY<>1