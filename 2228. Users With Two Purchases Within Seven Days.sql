...
SELECT DISTINCT USER_ID FROM ( 
    SELECT 
        USER_ID, 
        COUNT(purchase_id) OVER(PARTITION BY USER_ID ORDER BY purchase_date RANGE BETWEEN INTERVAL 7 DAY PRECEDING AND CURRENT ROW) AS NUM_PURCHASE 
FROM Purchases) AS A
WHERE NUM_PURCHASE=2
