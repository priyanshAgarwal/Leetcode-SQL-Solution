...
WITH CTE AS (
    SELECT *, 
    ID-DENSE_RANK() OVER(ORDER BY ID) AS GROUP_STADIUM FROM STADIUM (-- DENSE_RANK WILL ONLY RETURN PEOPLE WITH GREATER THAN 99 SO WHEN PEOPLE IS LESS THEN THERE WILL BE DIFFERENCE DENSE_RANK AND ID)
    WHERE PEOPLE>99
)

SELECT ID, VISIT_DATE, PEOPLE FROM CTE WHERE GROUP_STADIUM IN (SELECT GROUP_STADIUM FROM CTE
GROUP BY GROUP_STADIUM
